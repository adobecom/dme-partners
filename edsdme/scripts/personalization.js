...
const PAGE_PERSONALIZATION_PLACEHOLDERS = {
...
function removeElement(element, conditions) {
  ...
  const removeElement = conditions.every((condition) => {
    const conditionLevel = condition.startsWith(LEVEL_CONDITION) ? condition.split('-').pop() : '';
    return conditionLevel ? !PERSONALIZATION_CONDITIONS[LEVEL_CONDITION](conditionLevel) : !PERSONALIZATION_CONDITIONS[condition];
  });
  ...
  [...blocks, ...sections].forEach((el) => {
    ...
    if(el.classList.contains('section-metadata')) {
      ...
      Array.from(el.children).forEach((child) => {
        ...
        conditions = col2?.textContent?.split(',').map((el) => el.trim());
      });
    }
